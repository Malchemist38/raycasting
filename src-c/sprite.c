...
void renderMapSprites() {
    for (int i = 0; i < NUM_SPRITES; i++) {
        drawRect(sprites[i].x * MINIMAP_SCALE_FACTOR,
                ...
                2,
                2,
                (sprites[i].isVisible) ? 0xFF00FFFF: 0xFF444444 // yellow or dark grey
                );
    }
}
...
void renderSpriteProjection() {
    ...
    // find visible sprites (inside FOV)
    for (int i = 0; i < NUM_SPRITES; i++) {
    ...
    // sort sprites by distance - "painter's algorithm" using bubble sort
    ...
    for (int i = 0; i < numVisibleSprites; i++) {
        ...
        // calculate the perpendicular distance of the sprite to avoid fish-eye distortion
        float perpDistance = sprite.distance * cos(sprite.angle);

        // calculate the sprite projected height and width - all sprites are square
        float spriteHeight = (TILE_SIZE / perpDistance) * DIST_PROJ_PLANE;
        float spriteWidth = spriteHeight;

        // same formulas as wall top and bottom
        ...
        for (int x = spriteLeftX; x < spriteRightX; x++) {
            ...
            for (int y = spriteTopY; y < spriteBottomY; y++) {
                if (x > 0 & x < WINDOW_WIDTH && y > 0 && y < WINDOW_HEIGHT) {
                    ...
                    if (sprite.distance < rays[x].distance && texelColor != 0xFFFF00FF) { // don't draw sprites behind walls and ignore magenta background
